...
export default class guides extends base {
  ...
  static guideList(categories) {
    const url = `${this.baseUrl2}/work/zwzx/workGuide.do`;
    return new Page(url, this._processGuidesListItem.bind(this,categories));
  }

  static _processGuidesListItem(categories,guide) {
    const id=guide.management;
    guide.managementName = categories.find(item => item.id == id).title;
  }
   ...
   * 商品详情
   */
  static async detail(guideId) {
    const url = `${this.baseUrl2}/work/zwzx/queryOneGuides.do?guideId=${guideId}`;
   ...
   * 获取流程
   */
  static async flow(guideId) {
    const url = `${this.baseUrl2}/work/zwzx/workFlow.do?guideId=${guideId}`;
    const data = await this.get(url);
    let flowList;
    flowList = data.map(item => {
      const flowName = item.flowName;
      const flowContent = item.flowContent;
      const flowChart = item.flowChart;
      return {flowName, flowContent, flowChart};
    });
...
//    return data;
   ...
   * 获取材料
   */
  static async material(guideId) {
    const url = `${this.baseUrl2}/work/zwzx/workMaterial.do?guideId=${guideId}`;
    const data = await this.get(url);
    let materialList;
    materialList = data.map(item => {
      const id=item.id;
      const materialName = item.materialName;
      const materialContent = item.materialContent;
      const materialStandard = item.materialStandard;
      const materialRequired = item.materialRequred;
      const applyMaterial=item.applyMaterial;
      return {id,materialName, materialContent, materialStandard,materialRequired,applyMaterial};
    });
